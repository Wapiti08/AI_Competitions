...
clf = MLPClassifier(random_state=1, max_iter=500).fit(train_x, train_y_2)
...
pre_df.to_csv('./Dataset/prediction2.csv', index=False)

'''
test_x = normalize(test_x, norm='l2') without timegap

20% accuracy

test_x = StandardScaler().fit_transform(test_x) without timegap

20% accuracy

test_x = MinMaxScaler().fit_transform(test_x) without timegap

21% accuracy

test_x = MinMaxScaler().fit_transform(test_x) with timegap
